...
%\documentclass[APA,STIXTWOTEXT]{WileyNJDv5} %STIX1COL,STIX2COL,STIXSMALL
...
From a Bayesian standpoint, we decide to be in favor of the alternative if the Bayes factor exceeds a given evidence threshold $\ugamma_{lk}$, for all $(l, k) \in \mathcal{P}$ \citep{kass1995bayes,lee2014bayesian,stefan2019tutorial}. 
The ranges of thresholds for Bayes factors and their interpretation are provided in many papers \citep[see][]{kass1995bayes,lee2014bayesian}. We choose to select the evidence thresholds $\ugamma_{lk}$ for our Bayes Factors to parallel frequentist tests \citep{johnson2013uniformly}. We provide a default way to choose the evidence threshold later. A Bayes factor computed based on a single RP matrix $\uPhi$ can be unstable and very sensitive to the choice of the single RP matrix. Instead, we base our final decision on multiple RPs using an ensemble test. Hence, for $N$ randomly chosen RPs matrices, with $N$ sufficiently large, our final test statistic based on the BF in \eqref{eq:BFmax} is defined as
% {\color{red}
\be
...
The alternative described above was also previously considered in \cite{srivastava2014raptt} and \cite{zoh2018powerful} %{\color{blue}[It looks like we are considering two alternatives here? If so it would be better if we state this explicitly before ``In case 1, only...'']}
...
\vspace*{12pt}
\bmsubsection*{Proof of Theorem 1}
\begin{proof}[\textbf{\upshape Proof:}]
...
It can be shown that the unconditional distribution of $f_{lk} /(1 + \eta_{lk}) \sim F_{m, n-m-(G-1)}$ \cite[see for reference][page 704]{johnson2005bayes}.
% {\color{black} 
...
\bmsubsection{Proof of Theorem 3}
