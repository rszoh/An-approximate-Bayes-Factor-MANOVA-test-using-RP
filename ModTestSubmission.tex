...
% \be
% H_0 &:&\; f_{12}\; \sim \; F(m, N-m-1, 0)\; \mbox{vs.} \; \nonumber\\
% H_1 &:& \; f_{12}\; \sim \; F(m, N-m-1, \lambda_{12})\;\; & & \;\; \lambda_{12}| \tau^2 \sim \mbox{G}(m/2 + 1, 1/(2\tau^2) )\;\; \label{eq:hyp1} 
%  %H_1: \; \exists (i,j) \in \mathcal{P}\;\; \mbox{s.t}\;\; \udelta_{ij} &\sim& \MVN_{p}(\uzero, \uSigma/\tau_{0,ij})\; \text{and}\; \uSigma \propto |\uSigma|^{-(p+1)/2} \; 
% \ee

\begin{flalign}
H_0 &:  f_{12,u}\; \sim \; F(m, n_1+n_2-m-1, 0)\; \mbox{vs.} \; \\ \nonumber
H_1 &:  f_{12,u} \; \sim \; F(m, n_1+n_2-m-1, \lambda_{12}), \;\; \lambda_{12,\phi}| \tau_u^2 \sim \mbox{Gamma}\left(\frac{m}{2} + 1, \frac{1}{2\tau_u^2} \right), 
...
H_1 &:  f_{lk,u} \; \sim \; F(m, n_l+n_k-m-1, \lambda_{lk}), \;\; \lambda_{lk,\phi}| \tau_u^2 \sim \mbox{Gamma}\left(\frac{m}{2} + 1, \frac{1}{2\tau_{lk,u}^2} \right), 
% a_{ijk} &= \frac {Pr(M_{I} =2  \&  M_J=1 \& M_K =1 | I=i , J=j , K=k)}{Pr (M_I =1 \& M_J =1 \& M_K=1 | I=i , J=j)}&&\\\nonumber
%             &= \frac {\mu_{ijk211}}{\mu_{ijk111}}&&
\end{flalign}
