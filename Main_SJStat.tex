...
% \author[1\authfn{1}]{Joshua D. Trzasko, PhD}
...
\corraddress{Roger S Zoh PhD, Department of Epidemiology \& Biostatistics, Indiana University, Bloomington, IN 47405, USA}
\corremail{rszoh@iu.edu}
...
The ranges of thresholds for Bayes factors and their interpretation are provided in \citep{kass1995bayes,lee2014bayesian}. We choose to select the evidence thresholds $\ugamma_{lk}$ for our Bayes Factors to parallel frequentist tests \citep{johnson2013uniformly}. We provide a default way to choose the evidence threshold later. A Bayes factor computed based on a single RP matrix $\uPhi$ can be unstable and very sensitive to the choice of the single RP matrix. Instead, we base our final decision on multiple RPs using an ensemble test. Hence, for $N$ randomly chosen RPs matrices, with $N$ sufficiently large, our final test statistic based on the BF in \eqref{eq:BFmax} is defined as
% {\color{red}
\be
